...
if [ "$UNAME" = "Darwin" ]; then
  ...
  if [ "$UNAME" = "FreeBSD" ]; then
    ...
    if [ ! -f "glib-$GLIB/_build/gio/libgio-2.0.a" ] || [ ! -f "glib-$GLIB/_build/glib/libglib-2.0.a" ]; then
      ...
      (export PATH=$TPWD/ninja:$PATH; cd glib-$GLIB ; /usr/local/bin/meson _build -Ddefault_library=static -Dselinux=disabled -Dxattr=false -Dlibmount=disabled -Dinternal_pcre=true; ninja -C _build)
      if [ $? -ne 0 ]; then
